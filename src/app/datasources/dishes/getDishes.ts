...
export const getDishesByRestaurantIdPaginated = async (
  rid: number,
  page: number,
  size: number,
  token: string = ''
):Promise<PaginatedDishesResponseDto> => {
  const headers = new Headers();
  headers.append("Content-Type", "application/json");
  headers.append("Authorization", `Bearer ${token}`);  
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_PLAZA_HOST}/dishes/paginated?rid=${rid}&page=${page}&size=${size}`,
      {
        method: "GET",
        headers,
      }
